...
def get_pic_height(ascii_img):
    ...
    for i in range(scaled_img_height):
        for j in range(scaled_img_width):
            if(ascii_img[i][j] != ' ' and pic_hori_begin == 0):
                pic_hori_begin = i
                break
    for i in range(scaled_img_height):
        for j in range(scaled_img_width):
            if(ascii_img[scaled_img_height-1-i][j] != ' ' and pic_hori_end == 0):
    ...
    for i in range(scaled_img_height):    #j:x   i:y
        for j in range(scaled_img_width):
            ...
            if(ascii_img[i][j] != ' '):
                ...
                if(i == pic_hori_begin + light_line or i == pic_hori_begin + light_line -1):
...
pic_height = get_pic_height(ascii_img)
...
while 1:
    ...
    #canvas.itemconfig(img_obj, image=tkimg)
    if(loop_count == 40):
        creat_gif(temp_img_list, "Kucodeart004.gif")
