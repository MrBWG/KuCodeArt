...
function preload(){
	...
	imageCanary = loadImage('./moonriver400s.png');
	imageCanaryL = loadImage('./moonriver160.png');
	imageCanaryR = loadImage('./moonriver250.png');
}
...
function draw() {
	...
	for (let i = 0; i < nvalue; i++){
		...
		else if(pow(vecLocation[i].x -(width*3/4),2)+pow(vecLocation[i].y -(height/5),2)<=22000){
			vecLocation[i].add(vecVelocity[i]);
			if(vecLocation[i].y-(height/5) > 0){
				ellipse(vecLocation[i].x, (height/5)+ sqrt(22000-pow(vecLocation[i].x-(width*3/4),2)), size[i], size[i]);
				if(baseLocation[i].y > height/2){
					if(pow(vecLocation[i].x -(width*3/4),2)+pow(baseLocation[i].y -(height/5),2)>=22000){
						if(pow(vecLocation[i].x -(width/4),2)+pow(baseLocation[i].y -(height*4/5),2)>=22000){
		...
		}else if(pow(vecLocation[i].x -(width/4),2)+pow(vecLocation[i].y -(height*4/5),2)<=10000){
			vecLocation[i].add(vecVelocity[i]);
			if(vecLocation[i].y-(height*4/5) > 0){
				ellipse(vecLocation[i].x, (height*4/5)+ sqrt(10000-pow(vecLocation[i].x-(width/4),2)), size[i], size[i]);
			}else{
				ellipse(vecLocation[i].x, (height*4/5)- sqrt(10000-pow(vecLocation[i].x-(width/4),2)), size[i], size[i]);
				if(baseLocation[i].y < height/2){
					if(pow(vecLocation[i].x -(width*2/4),2)+pow(baseLocation[i].y -(height/5),2)>=10000){
						if(pow(vecLocation[i].x -(width/4),2)+pow(baseLocation[i].y -(height*4/5),2)>=10000){
							ellipse(vecLocation[i].x, baseLocation[i].y, size[i], size[i]);   
						}
					}
				}
	...
	image(imageCanary, width/2-195, height/2-205, imageCanary.width, imageCanary.height);
	image(imageCanary, width/4-80, height*4/5-80, 160, 160);
	image(imageCanary, width*3/4-125, height/5-125, 250, 250);
	if(runCount > 200) {
		drawLight(imageCanary, width/2-195, height/2-205, imageCanary.width, imageCanary.height, lightIndex);
		drawLight(imageCanaryL, width/4-80, height*4/5-80, 160, 160, lightIndex);
		drawLight(imageCanaryR, width*3/4-125, height/5-125, 250, 250, lightIndex);
	}
